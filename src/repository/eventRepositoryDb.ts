...
export async function addEvent(newEvent: Event): Promise<Event> {
  ...
  const [result] = await connection.execute('INSERT INTO events (category, title, description, location, date, time, petsAllowed, organizer) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [category, title, description, location, date, time, petsAllowed, organizer]);
